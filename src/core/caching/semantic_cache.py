...
class SemanticCache:
    ...
    def __init__(
        ...
        Args:
            ...
            reranker: Cross-encoder reranker for Layer 3 validation (auto-loaded from config if None)
            similarity_threshold: Threshold for semantic match (defaults to config)
            rerank_threshold: Threshold for cross-encoder validation (defaults to config)
        ...
        self.rerank_threshold = rerank_threshold if rerank_threshold is not None else settings.cache.rerank_threshold
        self.ttl = ttl_seconds if ttl_seconds is not None else settings.cache.semantic_cache_ttl
        self.max_size = max_cache_size
        self.prefix = prefix

        # Layer 3: Initialize cross-encoder from config if no reranker provided
        if reranker is not None:
            ...
            try:
                ...
                logger.warning(f"Failed to load cross-encoder '{settings.cache.cross_encoder_model}': {e}. Layer 3 disabled.")
                self.reranker = None
        ...
        logger.info(
            ...
            f"ttl={self.ttl}s, reranker={'yes' if self.reranker else 'no'}"
        )
    ...
    async def get(self, query: str) -> CacheResult:
        ...
        query_embedding = np.array(await self.embed_func(query))
    ...
    async def set(
        ...
        try:
            embedding = await self.embed_func(query)
        except Exception as e:
